...
mongoose
  .connect(
    "mongodb+srv://siddhantkankaria2001:sid1034@cooking.jk41rt9.mongodb.net/CookingBlogDB",
    {
...
// Category mongoose coding
const categorySchema = new mongoose.Schema({
  name: { type: String, required: true },
  image: { type: String, required: true },
});
const Category = mongoose.model("Category", categorySchema);
const recipeOne = new Category({
  name: "Thai",
  image: "thai-food.jpg",
});
const recipeTwo = new Category({
  name: "American",
  image: "american-food.jpg",
});
const recipeThree = new Category({
  name: "Chinese",
  image: "chinese-food.jpg",
});

async function insertDummyCategortyData() {
  try {
    await Category.insertMany(defaultRecipes)
      .then(function () {
...
insertDummyCategortyData();
app.get("/", async function (req, res) {
  try {
    const limitNumber = 5;
    const categories = await Category.find({}).limit(limitNumber);
    const recipeLatestOne = await Recipe.find({})
      .sort({ _id: -1 })
      .limit(limitNumber);
    const AmericanRecipe = await Recipe.find({ category: "American" }).sort({
      _id: 1,
    });
    const ChineseRecipe = await Recipe.find({ category: "Chinese" }).limit(
      limitNumber
    );
    const ThaiRecipe = await Recipe.find({ category: "Thai" }).limit(
      limitNumber
    );
    // const food = { recipeLatest };
    res.render("index", {
      categories,
      recipeLatestOne,
      AmericanRecipe,
...
app.get("/recipe/:_id", async function (req, res) {
  const id = req.params._id;
  try {
    const getID = await Recipe.findOne({ _id: id });
    console.log(typeof getID);
    if (getID != null) {
      res.render("recipe", { getID });
    }
  } catch {}
});
...
app.post("/search", async function (req, res) {
  try {
    ...
    res.send("search try not");
  }
});
app.get("/explore-latest", async function (req, res) {
  try {
    const limitNumber = 20;
    const Exploreall = await Recipe.find({})
      .sort({ _id: -1 })
      .limit(limitNumber);
    res.render("explore-latest", { Exploreall });
  } catch {}
});
...
//       res.json(recipe);
//     } else {
//       res.send("No recipe found matching the search term.");
//     }
//   } catch (error) {
//     res.send("Search operation failed: " + error.message);
//   }
// });
// latest recipe moongoose coding
...
const Recipetweleve = new Recipe({
...
const Recipethirteen = new Recipe({
...
const Recipefourteen = new Recipe({
  name: "Key lime pie",
  description:
    "\n        Preheat the oven to 175ºC/gas 3. Lightly grease a 22cm metal or glass pie dish with a little of the butter.\n        For the pie crust, blend the biscuits, sugar and remaining butter in a food processor until the mixture resembles breadcrumbs.\n        Transfer to the pie dish and spread over the bottom and up the sides, firmly pressing down.\n        Bake for 10 minutes, or until lightly browned. Remove from oven and place the dish on a wire rack to cool.\n        For the filling, whisk the egg yolks in a bowl. Gradually whisk in the condensed milk until smooth.\n        Mix in 6 tablespoons of lime juice, then pour the filling into the pie crust and level over with the back of a spoon.\n        Return to the oven for 15 minutes, then place on a wire rack to cool.\n        Once cooled, refrigerate for 6 hours or overnight.\n        To serve, whip the cream until it just holds stiff peaks. Add dollops of cream to the top of the pie, and grate over some lime zest, for extra zing if you like.\n\n       Source: https://www.jamieoliver.com/recipes/fruit-recipes/key-lime-pie/",
  email: "hello@email.com",
